...
export function Block({ block }: { block: ContentBlock }) {
  switch (block.kind) {
    ...
    case 'table':
      return (
        <div className="overflow-x-auto rounded-[12px] border-[0.5px] border-[rgb(var(--rule)/0.14)]">
          <table className="w-full text-left text-[13px]">
            <thead>
              <tr className="bg-[rgb(var(--rule)/0.03)]">
                {block.columns.map((c) => (
            ...
            <tbody>
              {block.rows.map((r, ri) => (
                <tr key={ri} className="border-t-[0.5px] border-[rgb(var(--rule)/0.08)]">
                  {r.map((cell, ci) => (
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    !showState && chosen && 'border-ink-primary bg-[rgb(var(--rule)/0.04)]',
                    !showState && !chosen && 'border-[rgb(var(--rule)/0.12)] hover:bg-[rgb(var(--rule)/0.02)]',
    ...
    <div className="max-w-[760px] mx-auto">
      ...
      <div className="flex items-center gap-3 mb-4">
        <button
          ...
          className="shrink-0 w-8 h-8 rounded-full border border-[rgb(var(--rule)/0.15)] flex items-center justify-center text-ink-secondary hover:text-ink-primary hover:bg-black/[0.03] transition-colors"
        >
          <ArrowLeft size={16} />
        </button>
      ...
      {/* tabs */}
      <div className="mt-6 flex gap-1 border-b border-[rgb(var(--rule)/0.1)]">
      ...
      {(() => {
        ...
        return (
          <div className="mt-8 flex items-center justify-between gap-3 border-t border-[rgb(var(--rule)/0.08)] pt-4">
            {prev ? (
              <button
                type="button"
                onClick={() => onOpenModule?.(prev.id)}
                className="group inline-flex items-center gap-2 rounded-full border-[0.5px] border-[rgb(var(--rule)/0.16)] pl-2.5 pr-3.5 py-1.5 text-[12px] text-ink-secondary hover:border-accent-copper hover:text-accent-copper transition-colors max-w-[46%]"
            ...
            {next ? (
              <button
                ...
                className="group inline-flex items-center gap-2 rounded-full border-[0.5px] border-[rgb(var(--rule)/0.16)] pl-3.5 pr-2.5 py-1.5 text-[12px] text-ink-secondary hover:border-accent-copper hover:text-accent-copper transition-colors max-w-[46%]"
...
export default function BdAcademy() {
  ...
  return (
    <div className="max-w-[900px] mx-auto space-y-8">
      <section className="pb-6 border-b border-[rgb(var(--rule)/0.08)]">
        <Link
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2.5">
          ...
          <Link
            ...
            className="inline-flex items-center gap-2 rounded-full border border-[rgb(var(--rule)/0.16)] px-4 py-2 text-[13px] font-semibold text-ink-secondary hover:text-ink-primary hover:bg-black/[0.03] transition-colors"
          >
        ...
        {BD_MODULES.map((m) => {
          ...
          return (
            <button
              ...
              className="text-left rounded-[12px] border-[0.5px] border-[rgb(var(--rule)/0.14)] bg-cream p-4 hover:shadow-card transition-shadow flex items-start gap-3"
            >
