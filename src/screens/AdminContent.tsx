  ...
  return (
    <span
      className={cn(
        ...
        STATUS_STYLES[status as ContentStatus] ?? 'bg-danger-bg text-danger-fg',
      )}
    >
...
function VersionsPanel() {
  ...
  return (
    <div className="bg-cream rounded-2xl border border-[rgb(var(--rule)/0.08)] overflow-hidden">
      <header className="px-5 py-4 border-b border-[rgb(var(--rule)/0.06)]">
            ...
            <div className="flex gap-1.5 shrink-0">
              {v.status === 'draft' && (
                <button
                  ...
                  className="text-[11px] font-medium rounded-full border border-[rgb(var(--rule)/0.15)] px-2.5 py-1 hover:bg-black/[0.03] transition"
                >
                  Send to review
                </button>
              )}
              ...
              {v.status === 'published' && (
                <button
                  ...
                  className="text-[11px] font-medium rounded-full border border-danger/30 text-danger-fg px-2.5 py-1 hover:bg-danger-bg transition"
                >
                  Retire
                </button>
              )}
...
function ReviewQueuePanel() {
  ...
  return (
    <div className="bg-cream rounded-2xl border border-[rgb(var(--rule)/0.08)] overflow-hidden">
      ...
      <div className="px-5 py-4 flex flex-wrap items-center gap-2 border-b border-[rgb(var(--rule)/0.06)]">
        <select
          value={competencyId}
          onChange={(e) => setCompetencyId(e.target.value)}
          className="flex-1 min-w-[220px] bg-parchment border border-[rgb(var(--rule)/0.12)] rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-ink-primary"
        >
          {COMPETENCIES.map((c) => (
        ...
        {error && <p className="w-full text-[11px] text-danger-fg">{error}</p>}
      </div>
                ...
                {q.status === 'draft' && (
                  <>
                    ...
                    <button
                      ...
                      className="text-[11px] font-medium rounded-full border border-danger/30 text-danger-fg px-2.5 py-1 hover:bg-danger-bg transition inline-flex items-center gap-1"
                    >
              ...
              {isEditing ? (
                <div className="flex-1 flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="text-[11px] font-medium rounded-full border border-[rgb(var(--rule)/0.15)] px-3 py-1.5 hover:bg-black/[0.03] transition"
                  >
                    Cancel
                  </button>
                </div>
                ...
                <>
                  ...
                  <div className="flex gap-1.5 shrink-0">
                    <button
                      ...
                      className="inline-flex items-center gap-1 text-[11px] font-medium rounded-full border border-[rgb(var(--rule)/0.15)] px-2.5 py-1 hover:bg-black/[0.03] transition"
                    >
...
export default function AdminContent() {
  return (
    <div className="max-w-[900px] mx-auto space-y-6">
      <section className="pb-5 border-b border-[rgb(var(--rule)/0.08)]">
