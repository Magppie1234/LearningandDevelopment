...
function DocumentCard({
  ...
  return (
    <motion.div
      ...
      className="group bg-cream rounded-lg p-5 border border-[rgb(var(--rule)/0.08)] shadow-card hover:shadow-elevated hover:-translate-y-[3px] transition-all duration-300 flex flex-col"
    >
      ...
      {/* Footer actions */}
      <div className="flex items-center justify-between pt-3 border-t border-[rgb(var(--rule)/0.06)]">
  ...
  return (
    <div className="max-w-[1200px] mx-auto">
        ...
        >
          <div className="relative flex items-center">
            ...
            <input
              ...
              className="w-full h-14 bg-cream rounded-full pl-14 pr-[140px] text-base text-ink-primary placeholder:text-ink-tertiary border border-[rgb(var(--rule)/0.12)] focus:border-ink-primary focus:outline-none focus:ring-[3px] focus:ring-[rgb(var(--rule)/0.08)] transition-all"
            />
      ...
      <motion.section
        ...
        className="rounded-3xl bg-card border border-[rgb(var(--rule)/0.08)] shadow-card p-8 mb-10"
      >
        <div className="flex flex-col lg:flex-row items-center gap-6">
          {/* Left */}
          <div className="flex-1">
            <div className="w-12 h-12 rounded-full bg-accent-copper flex items-center justify-center mb-4">
              <Sparkles size={24} className="text-stone-ivory" />
            </div>
            <h2 className="font-serif text-[28px] font-normal text-ink-primary mb-2">
              Can&apos;t Find What You Need?
            </h2>
            <p className="text-base text-ink-secondary leading-relaxed max-w-[560px]">
          ...
          <div className="flex-1 flex flex-col items-end gap-4">
            <div className="flex flex-wrap gap-2 justify-end">
              {[
                ...
                <span
                  key={q}
                  className="text-xs text-ink-secondary bg-cream border border-[rgb(var(--rule)/0.08)] px-3 py-1.5 rounded-full"
                >
                  {q}
                </span>
              ))}
            ...
            <Link
              href="/ai-assistant"
              className="bg-accent-copper text-stone-ivory px-6 py-2.5 rounded-full text-sm font-semibold hover:brightness-95 transition-all inline-flex items-center gap-2"
            >
              Ask AI Assistant
              <ArrowRight size={16} />
            </Link>
        ...
        <div className="flex gap-4 overflow-x-auto hide-scrollbar pb-2">
          {popularDocuments.map((doc, i) => (
            <motion.div
              ...
              className="min-w-[220px] max-w-[220px] bg-cream rounded-lg p-4 border border-[rgb(var(--rule)/0.08)] shadow-card hover:shadow-elevated hover:-translate-y-1 transition-all duration-300 relative"
            >
      ...
      >
        ...
        <div className="bg-cream rounded-lg border border-[rgb(var(--rule)/0.08)] overflow-hidden">
          {/* Horizontal scroll rather than clipping: at 375px the four
              fixed columns need ~400px, and `overflow-hidden` alone silently
              hid Updated and By instead of letting them be reached. */}
          <div className="overflow-x-auto">
          {/* Table header */}
          <div className="grid grid-cols-[minmax(180px,1fr)_140px_120px_140px] gap-4 px-5 py-3 min-w-[620px] bg-parchment/60 border-b border-[rgb(var(--rule)/0.08)]">
          ...
          {recentlyUpdated.map((doc, i) => (
            <motion.div
              ...
              className="grid grid-cols-[minmax(180px,1fr)_140px_120px_140px] gap-4 px-5 py-3.5 min-w-[620px] border-b border-[rgb(var(--rule)/0.06)] hover:bg-[rgb(var(--rule)/0.03)] transition-colors items-center"
            >
