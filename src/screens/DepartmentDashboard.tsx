...
export default function DepartmentDashboard() {
  const { member, can } = useRole()
  const params = useSearchParams()

  /**
   * Default scope is the viewer's own department; `?dept=` from an executive
   * drill-down overrides it. A Head who cannot see the whole company is held
   * to their own department however the URL is set — §19's scope boundary is
   * enforced here, not just in the selector.
   */
  const requested = params?.get('dept')
  const own = member?.departmentSlug ?? 'business-development'
  const initial = requested && (can('view_company') || requested === own) ? requested : own
  const [slug, setSlug] = useState(initial)
  const canPickAny = can('view_company')
  const department =
    DEPARTMENTS.find((d) => d.slug === slug) ??
    DEPARTMENTS.find((d) => d.slug === own) ??
    DEPARTMENTS[0]
      ...
      <section className={cn(CARD, 'p-4 flex flex-col sm:flex-row sm:items-center gap-3')}>
        ...
        {canPickAny ? (
          <select
            ...
            className="flex-1 sm:flex-none sm:min-w-[280px] rounded-lg border border-hairline/15 bg-parchment px-3 py-2 text-sm text-ink-primary focus:outline-none focus:border-accent-copper"
          >
            {DEPARTMENTS.map((d) => (
          ...
          <p id="dept-select" className="text-sm font-medium text-ink-primary">
            {department.name}
          </p>
        )}
        <p className="text-[11px] text-ink-tertiary sm:ml-auto sm:text-right max-w-xs">
          ...
          Champion: {department.champion}.{' '}
          {canPickAny
            ? 'You can switch departments because your role covers the whole company.'
            : 'A Department Head sees only their own department (§19).'}
        </p>
      </section>
          ...
          {FUNNEL_STAGES.map(({ key, label, note }) => {
            ...
            return (
              <div key={key} className="flex items-center gap-2 sm:gap-3">
                ...
                <span
                  className="flex-1 min-w-0 h-5 rounded bg-[rgb(var(--rule)/0.05)] overflow-hidden"
                  title={note}
                >
                  <span
        ...
        <div className="space-y-2">
          {risk
            ...
            .map((r) => (
              <div
                key={r.competencyId}
                className="flex flex-wrap items-center gap-x-3 gap-y-1 border-b border-[rgb(var(--rule)/0.06)] last:border-0 pb-2 last:pb-0"
              >
                ...
                <span className="w-28 flex-shrink-0">
                  <span className="block h-2 rounded-full bg-[rgb(var(--rule)/0.08)] overflow-hidden">
                    <span
                      className={cn(
