...
import { Block as BdBlock } from '@/screens/BdAcademy'
  ...
  return (
    <span
      className={cn(
        ...
        status === 'Not Started' && 'bg-[rgb(var(--rule)/0.15)]'
      )}
    />
  )
}
...
function CourseActionButton({ status }: { status: CourseStatus }) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 text-xs font-medium text-ink-secondary bg-[rgb(var(--rule)/0.06)] px-3 py-1.5 rounded-full hover:bg-[rgb(var(--rule)/0.1)] transition-colors cursor-pointer">
      <Circle size={13} />
      Start
    </span>
  )
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative w-[280px]">
          ...
          <input
            ...
            className="w-full bg-cream border border-[rgb(var(--rule)/0.12)] rounded-lg pl-9 pr-3 py-2 text-sm text-ink-primary placeholder:text-ink-tertiary focus:outline-none focus:border-ink-primary transition-all"
          />
        </div>
        ...
        {filteredCourses.map((course, idx) => {
          ...
          return (
            <motion.div
              ...
              className="bg-cream rounded-xl border border-[rgb(var(--rule)/0.08)] overflow-hidden"
            >
              {/* Accordion Header */}
              <button
                onClick={() => toggleExpand(course.id)}
                className="w-full flex items-center gap-4 px-5 py-4 text-left hover:bg-[rgb(var(--rule)/0.02)] transition-colors"
              >
                {isExpanded ? (
                  <ChevronUp
                    size={18}
              ...
              <AnimatePresence>
                {isExpanded && (
                  ...
                  >
                    <div className="px-5 pb-5 pt-1 border-t border-[rgb(var(--rule)/0.06)]">
                        ...
                        <div>
                          ...
                          <ul className="space-y-2 mb-5">
                            {course.moduleBreakdown.map((mod, i) => (
                              <li
                                key={i}
                                className="flex items-center justify-between text-sm py-1.5 border-b border-[rgb(var(--rule)/0.04)] last:border-0"
                              >
                          ...
                          {/* Instructor */}
                          <div className="flex items-center gap-3 bg-[rgb(var(--rule)/0.03)] rounded-lg p-3">
        ...
        {modules.map((m) => {
          ...
          return (
            <button
              ...
              className="group w-full flex items-center gap-4 bg-surface-warm rounded-xl border border-[rgb(var(--rule)/0.10)] px-5 py-4 hover:bg-[#e2d6bf] hover:shadow-card transition-all text-left"
            >
              <span className="text-xs font-medium text-ink-tertiary w-6 flex-shrink-0">
              ...
              <span
                aria-hidden
                className="flex-shrink-0 w-8 h-8 rounded-full border border-[rgb(var(--rule)/0.15)] flex items-center justify-center text-ink-secondary group-hover:text-ink-primary transition-colors"
              >
                <ChevronDown size={16} />
              </span>
            </button>
      ...
      {openModule && (
        ...
        >
          <div className="w-full max-w-[1200px] max-h-[calc(100dvh-1rem)] sm:max-h-[calc(100dvh-2rem)] overflow-y-auto rounded-2xl bg-card shadow-2xl">
            <div className="sticky top-0 z-10 flex items-center gap-3 bg-card/95 backdrop-blur-sm border-b border-[rgb(var(--rule)/0.1)] px-5 py-4">
              <span className="text-xs font-medium text-ink-tertiary w-6 flex-shrink-0">
              ...
              <button
                ...
                aria-label="Close"
                className="flex-shrink-0 w-9 h-9 rounded-full border border-[rgb(var(--rule)/0.15)] flex items-center justify-center text-ink-secondary hover:text-ink-primary hover:bg-black/[0.03] transition-colors"
              >
                <ChevronUp size={18} />
              </button>
            </div>
...
function MyProgressTab({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[55%_45%] gap-6">
      ...
      <div className="bg-cream rounded-2xl border border-[rgb(var(--rule)/0.08)] p-8 shadow-card">
        <div className="flex flex-col items-center text-center">
          ...
          {/* Stats below ring */}
          <div className="grid grid-cols-3 gap-6 w-full mt-8 pt-6 border-t border-[rgb(var(--rule)/0.06)]">
            <div className="text-center">
          ...
          {/* Next Milestone */}
          {nextCourse && (
            <div className="mt-6 p-4 bg-[rgb(var(--rule)/0.03)] rounded-xl w-full text-left">
      ...
      <div className="bg-cream rounded-2xl border border-[rgb(var(--rule)/0.08)] p-6 shadow-card">
          ...
          {sortedCourses.map((course) => (
            <div key={course.id} className="flex items-center gap-3">
              <div className="flex-1 min-w-0">
                ...
                <div className="h-1 w-full bg-[rgb(var(--rule)/0.06)] rounded-full overflow-hidden">
                  <div
        ...
        {resources.map((res, idx) => {
          ...
          return (
            <motion.div
              ...
              className="bg-cream rounded-xl border border-[rgb(var(--rule)/0.08)] p-5 shadow-card hover:shadow-elevated hover:-translate-y-0.5 transition-all duration-300"
            >
              <div
                className={cn(
              ...
              <button className="mt-4 w-full text-center text-xs font-semibold text-ink-secondary bg-[rgb(var(--rule)/0.06)] hover:bg-[rgb(var(--rule)/0.1)] py-2 rounded-full transition-colors">
                {res.type === 'Video'
                  ? 'Watch'
                  : res.type === 'Link'
                    ? 'Open'
        ...
        {leaderboard.map((entry, idx) => (
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <div className="flex-1 h-1.5 bg-[rgb(var(--rule)/0.06)] rounded-full overflow-hidden">
                <div
    ...
    <div className="max-w-[1200px] mx-auto space-y-6">
      ...
      {(academy.id === 'business-development' || academy.id === 'sales') && (
        <Link
          ...
          className="group flex items-center justify-between gap-4 rounded-2xl border-[0.5px] border-[rgb(var(--rule)/0.14)] bg-cream px-6 py-5 hover:shadow-card transition-shadow"
        >
      ...
      <div className="sticky top-0 z-30 bg-parchment/95 backdrop-blur-sm border-b border-[rgb(var(--rule)/0.08)] -mx-8 px-8">
