...
interface ChatMessage {
  ...
  sources?: string[]
  /**
   * Null for the seeded welcome message. Stamping it with `new Date()` during
   * render made the server and client disagree on the printed time, which is a
   * hydration mismatch (React #418) on every load of this page.
   */
  timestamp: Date | null
}
...
function TypingIndicator() {
  return (
    ...
    >
      ...
      <div className="bg-parchment rounded-2xl rounded-tl-md px-5 py-4 border border-[rgb(var(--rule)/0.06)]">
    ...
    >
      ...
      <div className={cn('max-w-[75%]', isAI ? '' : 'items-end')}>        <div
          className={cn(
            ...
            isAI
              ? 'bg-parchment rounded-2xl rounded-tl-md text-ink-primary border border-[rgb(var(--rule)/0.06)]'
...
function SuggestedQueryCard({
  ...
  return (
    <motion.button
      ...
      onClick={onClick}
      className="bg-cream rounded-xl p-4 border border-[rgb(var(--rule)/0.08)] text-left hover:shadow-card hover:border-l-[3px] hover:border-l-ink-primary/30 transition-all duration-200 group"
    >
      <div
        className={cn(
...
function AIFeatureCard({
  ...
  return (
    <motion.div
      ...
      className="bg-cream rounded-xl p-6 border border-[rgb(var(--rule)/0.08)] text-center hover:shadow-card transition-all duration-300"
    >
      <div
        className={cn(
  ...
  return (
    <div className="max-w-[900px] mx-auto">
      ...
      <motion.section
        ...
        className="bg-cream rounded-3xl border border-[rgb(var(--rule)/0.08)] flex flex-col min-h-[520px] shadow-card mb-8"
      >
        {/* Message History */}
        <div
          ref={scrollRef}
        ...
        {/* Input Area */}
        <div className="border-t border-[rgb(var(--rule)/0.08)] p-4">
          <div className="flex items-center gap-3">
            ...
            <input
              ...
              // min-w-0 is load-bearing: without it the flex item refuses to
              // shrink below the intrinsic width of its placeholder, and the
              // whole input row overflows the viewport on a phone.
              className="flex-1 min-w-0 h-12 bg-parchment rounded-full px-5 text-sm text-ink-primary placeholder:text-ink-tertiary border border-[rgb(var(--rule)/0.08)] focus:border-ink-primary focus:outline-none focus:ring-[3px] focus:ring-[rgb(var(--rule)/0.08)] transition-all"
            />
