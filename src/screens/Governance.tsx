...
const SCOPE_STYLE: Record<string, string> = {
  ...
  operations: 'bg-[rgb(var(--rule)/0.08)] text-ink-primary',
}
...
export default function Governance() {
  return (
    <div className="max-w-4xl mx-auto px-5 py-8 space-y-8">
      ...
      {/* Learning cycle */}
      <section className="rounded-2xl bg-white/70 dark:bg-white/5 border border-[rgb(var(--rule)/0.08)] p-5">
      ...
      <section>
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          {GOVERNANCE_ROLES.map((r) => (
            <article key={r.id} className="rounded-2xl bg-white/70 dark:bg-white/5 border border-[rgb(var(--rule)/0.08)] p-4">
      ...
      <section>
        ...
        <div className="space-y-2">
          {GOVERNANCE_CADENCE.map((c) => (
            <div
              key={c.id}
              className="rounded-2xl bg-white/70 dark:bg-white/5 border border-[rgb(var(--rule)/0.08)] px-4 py-3 flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-4"
            >
      ...
      <section className="grid md:grid-cols-2 gap-4">
        <div className="rounded-2xl bg-white/70 dark:bg-white/5 border border-[rgb(var(--rule)/0.08)] p-5">
          <h2 className="flex items-center gap-2 text-sm font-semibold text-ink-primary mb-3">
          ...
          <ol className="space-y-1.5">
            {CONTENT_WORKFLOW.map((step, i) => (
              <li key={step} className="text-xs text-ink-secondary flex items-center gap-2">
                <span className="w-5 h-5 rounded-full bg-[rgb(var(--rule)/0.08)] text-ink-primary flex items-center justify-center text-[10px] font-semibold flex-shrink-0">
                  {i + 1}
                </span>
                {step}
              </li>
