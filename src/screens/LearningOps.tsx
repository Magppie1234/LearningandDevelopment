...
const card = 'rounded-2xl bg-white/70 dark:bg-white/5 border border-[rgb(var(--rule)/0.08)]'
      ...
      <div className="space-y-3">
        {KPI_DICTIONARY.map((k) => (
          <article key={k.id} className={cn(card, 'p-5')}>
            ...
            <p className="mt-2 text-sm font-mono text-ink-primary bg-[rgb(var(--rule)/0.05)] rounded-lg px-3 py-2 overflow-x-auto">
              {k.formula}
            </p>
      ...
      <div className={cn(card, 'p-5')}>
        ...
        <div className="flex flex-wrap gap-1.5">
          {DASHBOARD_FILTERS.map((f) => (
            <span key={f} className="text-xs px-2.5 py-1 rounded-full bg-[rgb(var(--rule)/0.06)] text-ink-secondary">
              {f}
            </span>
          ))}
        </div>
          ...
          <ol className="mt-3 flex flex-wrap items-center gap-1.5">
            {w.steps.map((s, i) => (
              <li key={s} className="flex items-center gap-1.5">
                <span className="text-xs px-2.5 py-1 rounded-full bg-[rgb(var(--rule)/0.06)] text-ink-secondary">
                  {s}
                </span>
      ...
      <div className={cn(card, 'p-5')}>
        ...
        <div className="space-y-2">
          {THRESHOLD_RECOMMENDATIONS.map((t) => (
            <div key={t.id} className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-4 border-b border-[rgb(var(--rule)/0.06)] last:border-0 pb-2 last:pb-0">
      ...
      <div className={cn(card, 'p-5')}>
        ...
        <div className="flex flex-wrap gap-1.5">
          {ASSESSMENT_TYPES.map((t) => (
            <span key={t} className="text-xs px-2.5 py-1 rounded-full bg-[rgb(var(--rule)/0.06)] text-ink-secondary">
              {t}
            </span>
          ))}
        </div>
