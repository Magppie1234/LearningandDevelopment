    ...
    <div
      className={cn(
        ...
        isCapping
          ...
          : 'border-[rgba(0,59,70,0.08)] bg-[rgba(0,59,70,0.03)]',
      )}
    >
...
function CompetencyRow({
  ...
  return (
    <div className={cn(CARD, 'overflow-hidden')}>
      ...
      {open && (
        <div className="px-4 pb-4 border-t border-[rgba(0,59,70,0.06)] space-y-3 pt-3">
...
export default function SkillsPassport() {
  const member = memberById(DEMO_USER_ID)!
    ...
    <div className="max-w-4xl mx-auto px-5 py-8 space-y-6">
      <header>
        ...
        <p className="mt-2 text-sm text-ink-secondary max-w-2xl">
          Your verified capability against what the role requires. Watching a lesson and passing a
          quiz are recorded here — but neither one alone makes a competency validated.
        </p>
      </header>
      ...
      <section className={cn(CARD, 'p-5')}>
        ...
        <div className="mt-3 grid sm:grid-cols-2 gap-2">
          {CHANNEL_ORDER.map((c) => (
            <div key={c} className="rounded-xl border border-[rgba(0,59,70,0.08)] px-3 py-2">
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {PROFICIENCY_LABEL.map((label, i) => (
            <span
              key={label}
              className="text-[11px] px-2 py-0.5 rounded-full bg-[rgba(0,59,70,0.06)] text-ink-secondary"
            >
              {i} · {label}
            </span>
          ))}
